...
// 返回值：   UINT-每列像素所占字节数
//=======================================================
UINT CDib::GetColByte()
{ 
	return (GetHeight() * GetBitCount() / 8 + 3) / 4 * 4;
}
...
BOOL CDib::MakePalette()
{
	...
	lpLogPalette->palNumEntries = (unsigned short)dwNumOfColor;
...
// 返回值：   无
//=======================================================
int CDib::Rand(int a,int b)
{
	return (int)(a + rand()*abs(b-a));
}
//=======================================================
// 函数功能： 创建一个新的位图
...
{	
	...
	m_bValid = FALSE;
    // 计算位图数据所需空间
    UINT uGradeBmpLineByte = (lWidth*nBits/8 + 3) / 4 * 4;
    DWORD dwGradeBmpDataSize = uGradeBmpLineByte * lHeight; 
    ...
    lpGradeBmpInfoHeader->biClrImportant = 0;
    lpGradeBmpInfoHeader->biClrUsed = 256;
    lpGradeBmpInfoHeader->biCompression = 0;
    lpGradeBmpInfoHeader->biHeight = lHeight;
    lpGradeBmpInfoHeader->biPlanes = 1;
    lpGradeBmpInfoHeader->biSize = sizeof(BITMAPINFOHEADER);
    lpGradeBmpInfoHeader->biSizeImage = dwGradeBmpDataSize;
    lpGradeBmpInfoHeader->biWidth = lWidth;
    lpGradeBmpInfoHeader->biXPelsPerMeter = 0; //m_lpBmpInfoHeader->biXPelsPerMeter;
    lpGradeBmpInfoHeader->biYPelsPerMeter = 0;//m_lpBmpInfoHeader->biYPelsPerMeter;
    ...
    for(int i = 0; i < lHeight; i++)
    {
        ...
        {						
			if(nBits == 24)
			{
				...
				lpGradeBmpData[i * uGradeBmpLineByte + 3 * j + 1] = Rand(90,180);
				lpGradeBmpData[i * uGradeBmpLineByte + 3 * j + 2] = Rand(180,256);
			}
			else
...
//=======================================================
// 函数功能： 根据已有数据创建一个新的位图
// 输入参数： fileName:图像文件名；m_pData:指向图像数据的指针；lWidth:图像宽度；lHeight:图像高度；nBits:像素位数
// 返回值：   无
//=======================================================
BOOL CDib::CreateDIB(char *fileName,int *m_pData,DWORD lWidth,DWORD lHeight,int nBits,int IsFitsFile)
{
    ...
    // 对数据区域赋值
	if(1 != IsFitsFile)
		memcpy(lpGradeBmpData,m_pData,uGradeBmpLineByte*lHeight);
	else
	{
		for(int i = 0; i < lHeight; i++)
			for(int j = 0; j < lWidth; j++)							
					lpGradeBmpData[i * uGradeBmpLineByte + j] = (BYTE)m_pData[i * uGradeBmpLineByte + j];
    }
	/*for(int i = 0; i < lHeight; i++)
    {
        for(int j = 0; j < lWidth; j++)
        {			
			if(nBits == 24)//图像为彩色图像
			{				
				lpGradeBmpData[i * uGradeBmpLineByte + 3 * j] = (BYTE)m_pData[i * uGradeBmpLineByte + 3 * j];
				lpGradeBmpData[i * uGradeBmpLineByte + 3 * j + 1] = (BYTE)m_pData[i * uGradeBmpLineByte + 3 * j + 1];
				lpGradeBmpData[i * uGradeBmpLineByte + 3 * j + 2] = (BYTE)m_pData[i * uGradeBmpLineByte + 3 * j + 2];
			}
			else//图像为灰度图像				
				lpGradeBmpData[i * uGradeBmpLineByte + j] = (BYTE)m_pData[i * uGradeBmpLineByte + j];
        }
